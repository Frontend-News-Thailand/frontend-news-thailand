  ...
  <div class="flex flex-col gap-6 pt-6">
    <ContentDoc v-slot="{ doc }">
      ...
      <div class="flex items-center gap-4">
        <img width="120" class="rounded-full" :src="getAuthor(doc.username)?.avatar" alt="author-image">

        <div class="flex flex-col gap-2">
          <div class="flex gap-4 items-center">
            <span class="text-xl">{{ getAuthor(doc.username)?.username }}</span>
          ...
          <span class="text-sm md:text-base font-light">{{ getAuthor(doc.username)?.bio }}</span>
        </div>
      </div>
    </ContentDoc>
  </div>
